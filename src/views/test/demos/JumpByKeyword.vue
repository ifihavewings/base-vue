<template>
  <div>
    <input v-model="searchText" placeholder="Enter search keyword" @keydown.enter="searchKeyword" />
    <button @click="searchKeyword">Search</button>
    <button @click="scroll">next</button>
    <div ref="content" style="height: 300px; overflow-y: scroll" v-html="articleBody"></div>
  </div>
</template>
...
const elements = ref<NodeListOf<Element>>()
const content = ref<HTMLDivElement>()
function searchKeyword() {
  const regex = new RegExp(searchText.value, 'gi')
  const regex1 = /<span class="highlight">([^<]+)<\/span>/g;
  articleBody.value = articleBody.value.replace(regex1, '$1');

  articleBody.value = articleBody.value.replace(regex, (match) => {
    let a = `<span class="highlight">${match}</span>`
    return a
  })
}
...
const counter = ref(0)
function scroll() {
  elements.value = content.value?.querySelectorAll('.highlight')  as NodeListOf<Element> ?? []
  max.value = elements.value.length
  if (counter.value < max.value) {
    (elements.value[counter.value++] as HTMLDivElement).scrollIntoView({ behavior: 'smooth' })
  } else {
    counter.value = 0;
    (elements.value[counter.value] as HTMLDivElement).scrollIntoView({ behavior: 'smooth' })
  }
